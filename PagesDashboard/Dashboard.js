...
export default class Dashboard extends React.Component {
  constructor() {
    ...
    if (localStorage.getItem("token")) {
      if (
        ...
        window.alert("You're not admin or kasir!2");
        window.location = "/";
      }
    }
  }
  ...
  checkRole = () => {
    if (this.state.role !== "admin" && this.state.role !== "kasir" && this.state.role !== "manajer") {
      localStorage.clear();
      window.alert("You're not admin or kasir!3");
      window.location = "/";
    }
  };
    ...
    <div class="flex flex-col justify-center md:pr-8 xl:pr-0 lg:max-w-lg">
      <div class="max-w-xl mb-6">
        <h2 class="max-w-lg mb-6 font-sans text-3xl font-bold tracking-tight text-blue-900 sm:text-4xl sm:leading-none">
          Welcome to Dai Nippon Cafe<br class="hidden md:block" />
        </h2>
        <p class="text-base text-gray-700 md:text-lg">
        Welcome to Dai Nippon cafe. Where tradition stays still.
        </p>
      </div>
      <div>
      </div>
